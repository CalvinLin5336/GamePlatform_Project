...
public interface TeamPostRepository extends JpaRepository<TeamPost, Long> {
	@Query("select p from TeamPost p where (:keyword='' or lower(p.title) like lower(concat('%',:keyword,'%')) or lower(p.gameName) like lower(concat('%',:keyword,'%'))) and (:status is null or p.status=:status) order by p.createdAt desc")
