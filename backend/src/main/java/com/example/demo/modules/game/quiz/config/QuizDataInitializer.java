...
public class QuizDataInitializer implements CommandLineRunner {
	...
	private void initQuestionsIfEmpty() {
		...
		createQuestion(
			...
			"<p>卷ch3-ex15 關於對象引用與垃圾回收哪項說法正確?</p><p>正確答案A) The memory allocated for John objcet can reused in line 1.</p><p>//為John物件分配的記憶體可以在第1行中重複使用</p><p><br></p><p>物件生命週期與記憶追蹤:</p><p>Java 中「記憶體可以被再利用(reused)」意味著該物件失去所有強引用(Strong Reference)，進入符合垃圾回收條件(Eligible for GC)的狀態。</p><p>。執行createPeople()期間:</p><blockquote>建立 Jane 物件，區域變數指向 jane 指向它。</blockquote><blockquote>建立 John 物件，區域變數 john 指向它，且 John.child 指向它，且John.child指向Jane。</blockquote><blockquote>回傳 jane，因此 main 中的 person 變數接住 Jane。</blockquote><blockquote>方法結束後，區域變數john 消失。此時沒有任何變數指向John(雖然John牽著Jane，但外面沒人牽著John。</blockquote><p>。到達 //Line 1 :</p><blockquote>John 物件無法透過任何GC Root存取，符合GC回收條件，其記憶體可以被重複利用(選項A正確，選想D錯誤，因為main的person仍抓著Jane)</blockquote><p>。執行 createPerson(person) 到達 //Line 2 :</p><blockquote>建立 Jack 物件，其 child 指向傳入的 person (即 Jane) 。</blockquote><blockquote>回傳 Jack ，並賦值給main 的 person。</blockquote><blockquote>此時 person 指向 Jack，而 Jack.child 指向 Jane，兩者皆可達物件(Reachable) ，均不可被回收(選項 B、C錯誤)。</blockquote><p>。 後續執行:</p><blockquote>person.toString() 會呼叫 Jack.toString() → 印出 JackJane。</blockquote><p><br></p><p>聯想:</p><p>把引用關係想像成「攀岩抓安全繩」:</p><p>。 Jane 是綁在 main 登頂安全扣( person) 上的核心繩索。</p><p>。 John只是半路出現的小跟班: 他伸手抓住Jane，但沒有人去抓 John。攀岩結束離開該方法時，John直接摔落深淵(被GC標記回收)</p><p>。 Jack則是後來空降的大哥: 安全扣換成抓著 Jack，而 Jack手裡牢牢抓著Jane。全體掛載，誰也沒掉。</p><p><br></p><p>考試背誦口訣:</p><p>「進房建兩客，只帶一人走；沒帶出的丟，牽著不能留。」</p><p>。 進房見兩客，只帶一人走: 在createPeople 裡建了 Jane 和 John，但 return jane 只帶走了 Jane。</p><p>。沒帶出的丟: 沒被return 接住的John，出發法立刻變垃圾(Line1可回收)</p><p>。牽著不能留: Jack 抓著Jane、外面抓著Jack，鍊條完好一個都別想收(Line2 都活著)。</p>",
			30,
		...
		createQuestionWithFlags(
			...
			"<p>卷ch8-ex2 說明:</p><p>下列哪些方法在 ExcelorService 實例上不可用？（選兩個）</p><p>execute 只接收Runnable，Callable應通過submi或invoke方法提交，ExecutorService也沒有exit()，結束服務通常用shutdown()或shutdown()</p><p><br></p><p>A為什麼錯 ? <strong>ex</strong>ecute(Callable) </p><p>//不可用:execute 僅接收Runnable</p><p>說明:在ExecutorService(及其父介面 Executor)中，execute()方法只解收Running 參數(即 execute(Runnable))</p><p><br></p><p>D為什麼錯 ? <strong>ex</strong>it() </p><p>//不可用: 無此方法，關閉應使用shutdown</p><p>說明:ExecutorService完全沒有 exit()這個方法</p><p><br></p><p>補充:</p><p>B.shutdownNow() 可用:用於立即停止執行器</p><p>C.submit(Runnable) 可用: 提交Runnable並傳Future&lt;?&gt;</p><p>E.submit(Callable) 可用:提交Callable並回傳Future&lt;T&gt;</p><p>F.execute(Runnable) 可用:繼承自Executor介面</p><p><br></p><p>記憶:</p><p>execute()是單純叫員工去做事，不期待帶結果回來，因此接 Runnable。submit()是正式交件，可以收回Future，因此Runnable 或 Callable都能交</p><p><br></p><p>口訣:</p><p>【ex跑(Runnable) 不Call， sub 兩者通，停機shut，停機shut，ep 根本沒exit】</p><p><br></p><p>。execute只能跑(Run): execute()來自父介面 Executor，只接受 Runnable，沒有 execute(Callable) (即A錯誤)。</p><p>。 submit兩者通 : submit()支援 Runnable 與 Callable，並會回傳 Future (C、E都合法)</p><p>。 關閉用 shut: 關閉線呈池用 shutdown() 或 shutdownNow() (B合法)。</p><p>。 沒有 exit : Java 執行緒池完全沒有 exit() 方法，結束 JVM使用 System.exit() (即 D 錯誤)</p>",
			30,
