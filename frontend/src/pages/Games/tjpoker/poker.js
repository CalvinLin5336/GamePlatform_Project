...
$(document).ready(function() {
    ...
    var modeParameter=getParameter("modeCode") || getParameter("mode");
    modeParameter=modeParameter.toUpperCase();
    if(getParameter("modeCode")) {
        platformRoom=true;
        platformUserId=Number(getParameter("userId"));
        platformSeat=Number(getParameter("seat"));
        api="http://"+backendHost+"/api/games/poker";
        pokerSocketPath="/ws/games/poker";
    }
...
function join(mode, id) {
    ...
    if(platformRoom) {
        if(!platformUserId || !platformSeat) {
            showMessage("缺少房間玩家資料", true);
            return;
        }
        body.userId=platformUserId;
        body.seat=platformSeat;
    }
...
function cardImagePath(card) {
    ...
    return "../../../assets/Games/tjpoker/cards/card_a_"+suitCode+point+".png";
}
